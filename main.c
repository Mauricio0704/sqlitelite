...
PrepareStatus prepare_statement(InputBuffer *input_buffer, Statement *statement)
{
...
void execute_insert(Statement *statement, Table *table)
{
    ...
    table->records[table->records_length].email[sizeof(table->records[table->records_length].email) - 1] = '\0';

    strncpy(table->records[table->records_length].username,
            statement->record_to_insert.username,
            sizeof(table->records[table->records_length].username) - 1);
    table->records[table->records_length].username[sizeof(table->records[table->records_length].username) - 1] = '\0';
